...
- aesthetics: top 4 aesthetic categories summing to 100, chosen verbatim (exact spelling and case) from the aesthetic-taxonomy-v2 list below — 478 labels grouped into style families, each with a one-line definition.

  HOW TO CHOOSE: (1) Ground every label in the specific garments, colors, and silhouette you can actually see in the photo — not a vague overall impression. (2) Search narrow before you search broad: a small set of labels are broad umbrella terms that superficially fit almost any outfit (Eclectic, Classic, Coastal cool, Streetwear, Minimalist, Business casual, Romantic, Understated, Preppy, Vintage prep) — these are last resorts, not defaults. Before picking one, scan the rest of its family for a more specific match (e.g. a relaxed outfit near water is "Coastal cool" only after ruling out "Nautical", "California casual", "Coastal grandmother", "Coastal cowgirl", "Surf girl"; a mixed-pattern outfit is "Eclectic" only after ruling out "Print mixing", "Color blocking", "Maximalist", "Art hoe"). (3) If you're uncertain what you're looking at, that uncertainty belongs in a lower confidence read on the outfit overall — never in picking a vaguer, safer-sounding label. (4) It's fine and correct for a genuinely plain, minimal outfit to score "Minimalist" or "Classic" — the goal is accuracy, not novelty for its own sake.
